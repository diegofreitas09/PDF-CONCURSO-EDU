...
for i in range(1,9):
    ...
    print(f'parte {i}: len={len(s)} sha={h} esperado={expected_parts[i]} ok={h==expected_parts[i]}')
    if len(s)!=10603 or h!=expected_parts[i]: sys.exit(f'fragmento divergente: parte {i}')
    parts.append(s)
try:
