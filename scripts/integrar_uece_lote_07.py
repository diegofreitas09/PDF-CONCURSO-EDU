...
# Se o payload estiver íntegro e apenas o checksum/rodapé XZ estiver corrompido,
# aproveite a saída recuperada e deixe as validações estruturais abaixo decidirem.
if raw is None:
    try:
...
if text.count('id:"UECE-PORT-INT-') != 100:
    sys.exit(f'payload recuperado não contém 100 IDs: {text.count(chr(105)+chr(100)+chr(58)+chr(34)+"UECE-PORT-INT-")} | '+ ' | '.join(errors))
