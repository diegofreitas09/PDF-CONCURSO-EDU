...
# Caminho normal: fluxo XZ íntegro.
try:
    raw=lzma.decompress(packed)
except Exception as e:
    errors.append(f'lzma={e}')

# Recuperação segura para XZ cujo índice/rodapé/checksum esteja danificado.
# Alimentamos incrementalmente e preservamos apenas bytes efetivamente
# descomprimidos antes do erro; as validações estruturais abaixo impedem
# que uma recuperação parcial seja promovida.
if raw is None:
    try:
        ...
        for pos,b in enumerate(packed):
            try:
                ...
                errors.append(f'lzma-incremental-pos={pos}:{e}')
                break
        candidate=b''.join(recovered)
        if candidate:
            raw=candidate
            errors.append(f'lzma-incremental-bytes={len(candidate)}')
    except Exception as e:
        errors.append(f'lzma-incremental={e}')

# Último fallback do sistema: xz --ignore-check.
if raw is None:
    try:
        ...
        if proc.stdout:
            ...
            if proc.returncode!=0:
                errors.append('xz-ignore-check-rc='+str(proc.returncode)+': '+proc.stderr.decode('utf-8','replace').strip())
        else:
...
# Não promova recuperação parcial: valide UTF-8 e estrutura completa antes de escrever.
try:
    text=raw.decode('utf-8')
except UnicodeDecodeError as e:
    sys.exit(f'payload recuperado não é UTF-8 íntegro: {e} | '+ ' | '.join(errors))
if 'export const UECE_PORTUGUES_LOTE_100_07' not in text:
    sys.exit('payload recuperado não contém o export esperado | '+ ' | '.join(errors))
count_ids=text.count('id:"UECE-PORT-INT-')
if count_ids != 100:
    sys.exit(f'payload recuperado não contém 100 IDs: {count_ids} | '+ ' | '.join(errors))
if 'UECE-PORT-INT-070' not in text or 'UECE-PORT-INT-169' not in text:
    sys.exit('payload recuperado não contém os limites 070–169 | '+ ' | '.join(errors))
# Exige fechamento sintático esperado para evitar promover arquivo truncado.
if '];' not in text[-500:]:
    sys.exit('payload recuperado não contém fechamento completo do lote | '+ ' | '.join(errors))
