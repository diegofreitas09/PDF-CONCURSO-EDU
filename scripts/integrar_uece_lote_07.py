...
import base64,lzma,sys,binascii
...
raw=None
errors=[]
# Formato A: um único base64 dividido em blocos de texto.
try:
    raw=lzma.decompress(base64.b64decode(''.join(parts), validate=True))
except Exception as e:
    errors.append(f'base64-continuo={e}')
# Formato B: cada bloco foi codificado em base64 separadamente.
if raw is None:
    try:
        packed=b''.join(base64.b64decode(p, validate=True) for p in parts)
        raw=lzma.decompress(packed)
    except Exception as e:
        errors.append(f'base64-por-bloco={e}')
if raw is None:
    details=[]
