...
export default function Simulados(){
 ...
 async function downloadReport(){
  if(!finished||!exam.length)return;
  setPdfLoading(true);setPdfError("");
  try{
   const doc=new jsPDF({unit:"mm",format:"a4"});
   const result=buildResult();
   const platform=loadPlatform();
   const name=platform?.settings?.name||"Aluno";
   const date=new Date(finishedAt||Date.now());
   const margin=16,pageW=210,pageH=297,usable=pageW-margin*2;
   let y=18;
   const ensure=(need=12)=>{if(y+need>pageH-16){doc.addPage();y=18}};
   const addText=(text,size=10,bold=false,space=4)=>{doc.setFont("helvetica",bold?"bold":"normal");doc.setFontSize(size);const lines=doc.splitTextToSize(String(text||""),usable);ensure(lines.length*(size*.38)+space);doc.text(lines,margin,y);y+=lines.length*(size*.38)+space};
   doc.setFillColor(20,31,48);doc.rect(0,0,210,38,"F");doc.setTextColor(255,255,255);doc.setFont("helvetica","bold");doc.setFontSize(20);doc.text("PDF CONCURSO EDU",margin,17);doc.setFontSize(12);doc.text("Relatório de Simulado",margin,27);
   doc.setTextColor(20,31,48);y=48;
   addText(`Aluno: ${name}`,11,true,2);
   addText(`Data: ${date.toLocaleDateString("pt-BR")} às ${date.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}`,10,false,2);
   addText(`Duração: ${formatDuration(result.durationSeconds)}`,10,false,5);
   doc.setFillColor(245,247,250);doc.roundedRect(margin,y,usable,28,3,3,"F");doc.setFont("helvetica","bold");doc.setFontSize(16);doc.text(`${result.correct}/${result.total} acertos`,margin+6,y+10);doc.setFontSize(12);doc.text(`Desempenho geral: ${result.accuracy}%`,margin+6,y+20);y+=36;
   addText("Desempenho por disciplina",13,true,4);
   Object.entries(result.byDiscipline).forEach(([d,r])=>addText(`${d}: ${r.correct}/${r.total} acertos — ${Math.round(r.correct/r.total*100)}%`,10,false,2));
   y+=4;addText("Detalhamento das questões",13,true,5);
   exam.forEach((q,i)=>{const selected=answers[q.instanceId],ok=selected===q.answer;ensure(32);doc.setDrawColor(ok?70:190,ok?150:70,ok?90:70);doc.setLineWidth(.5);doc.line(margin,y,margin+usable,y);y+=5;addText(`Questão ${i+1} — ${q.discipline} · ${q.topic||"Sem tópico"}`,11,true,3);if(q.context){addText("TEXTO-BASE",9,true,2);addText(q.context,8.5,false,3)}if(q.media)addText("[QUESTÃO COM APOIO VISUAL — consulte a versão interativa da plataforma]",8.5,true,3);addText(q.statement,10,false,3);q.options.forEach((o,oi)=>{const letter=String.fromCharCode(65+oi);let prefix=`${letter}) `;if(oi===selected)prefix+="[MARCADA] ";if(oi===q.answer)prefix+="[GABARITO] ";addText(prefix+o,9,false,2)});addText(`Resposta do aluno: ${selected===undefined?"Não respondida":String.fromCharCode(65+selected)} | Gabarito: ${String.fromCharCode(65+q.answer)} | ${ok?"ACERTOU":"ERROU"}`,9.5,true,3);if(q.explanation)addText(`Comentário: ${q.explanation}`,8.5,false,4)});
   ensure(20);doc.setDrawColor(180);doc.line(margin,y,margin+usable,y);y+=6;addText("Relatório gerado automaticamente pela plataforma PDF Concurso EDU.",8,false,2);addText("A ordem das questões e das alternativas é sorteada a cada novo simulado.",8,false,2);
   const safeDate=date.toISOString().slice(0,10);
   doc.save(`Relatorio_Simulado_PDF_Concurso_${safeDate}.pdf`);
  }catch(e){console.error("Erro ao gerar PDF",e);setPdfError(`Não foi possível gerar o PDF: ${e?.message||"erro desconhecido"}`)}finally{setPdfLoading(false)}
 }
